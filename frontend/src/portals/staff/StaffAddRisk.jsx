...
import { createRisk, updateRisk, getRisks } from "../../services/api";
import useAppStore from "../../store/useAppStore";

export default function StaffAddRisk() {
  const { id } = useParams();
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 md:p-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-xl font-black text-slate-900">{isEdit ? "Edit Risk" : "Report a Risk"}</h1>
            <p className="text-xs text-slate-400 mt-0.5">
              {isEdit ? `Updating ${initial?.risk_code ?? "risk"}` : `Logging for ${user?.department_name}`}
            </p>
          </div>
        </div>
